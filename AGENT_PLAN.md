...
**Post-Demo Day update (2026-08-01):** the roster below (§2) is the original 7-agent plan (Orchestrator + 5 research agents + Critic + Synthesis) and is kept as written — it's still accurate for those 7. Since Demo Day: a 6th research agent, **Patent Landscape** (EPO Open Patent Services), was added — same tier/execution model as the other 5, informational only (not in the Confidence Score, §5.1). Citation provenance is now enforced in code (a citation must trace back to a real tool/search result this run, not just pass schema validation) — this was previously a prompt-only guardrail. **Deep Research Mode** (opt-in) adds a conditional second pass: after Critic's first review, any research agent with a real flag against it gets one more targeted round, fed that specific flag, before Critic re-reviews and Synthesis runs — each agent's second pass is independent, not chained. A **batch queue** now lets multiple runs be submitted at once, sharing a concurrency-capped queue (2 full pipelines at a time) with the single manual "Run Assessment" trigger. The rest of this document — execution model, per-agent specs, guardrails, resolved open questions — is unchanged and still the source of truth for how the original 7 agents work; see `lib/agents/*.ts` inline comments for anything that's drifted since (schemas, prompts, and provenance logic are the actual source of truth, this document is the design rationale behind them).
