// Canonical agent roster — AGENT_PLAN.md §2's 7 tracked agents (Orchestrator
// coordinates but isn't itself tracked). Single source of truth for display
// order/labels so the pre-run agent list (therapy-profile-form.tsx) and the
// live progress screen (agent-progress.tsx) can't drift apart. `key` values
// match the `name` strings orchestrator.ts already passes to
// onAgentStatusChange — no translation layer needed between the two.
